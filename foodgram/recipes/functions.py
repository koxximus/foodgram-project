...
def check_ingredients(request):
    ...
    for key, value in request.POST.items():
        if "nameIngredient" in key:
            ...
            dimension = value
            if Ingredient.objects.filter(title=title, dimension=dimension).exists():
                ingredients.append([Ingredient.objects.get(title=title, dimension=dimension), amount])
            else:
                return False
    return ingredients
...
def create_ingredient_amount(ingredients, recipe):
    for ingredient, amount in ingredients:
        IngredientAmount.objects.create(amount=amount, recipe=recipe, ingredient=ingredient)
...
def pdf_create(ingredients):
    ...
    for key, ing in enumerate(ingredients):
        c.drawString(30, 60+25*key, f'{ing["ingredients__title"]}({ing["ingredients__dimension"]}) - {ing["amount"]}')
